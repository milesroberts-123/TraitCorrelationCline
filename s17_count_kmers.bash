...
#Count kmers in trimmed fastq files
while read -u 3 -r SRA; do
	if [ -f "$(echo $SRA)_1_trim.fastq" ]; then
    		#Count kmers for read1 and read2
        	echo Counting kmers for $SRA ...
        	./kmc -k31 -m16 -t$SLURM_CPUS_PER_TASK -ci10 -cs10 $(echo $SRA)_1_trim.fastq temporary1_$SLURM_ARRAY_TASK_ID tmp_$SLURM_ARRAY_TASK_ID
        	./kmc -k31 -m16 -t$SLURM_CPUS_PER_TASK -ci10 -cs10 $(echo $SRA)_2_trim.fastq temporary2_$SLURM_ARRAY_TASK_ID tmp_$SLURM_ARRAY_TASK_ID
    		...
    		#Count kmers for read1
                echo Counting kmers for $SRA ...
                ./kmc -k31 -m16 -t$SLURM_CPUS_PER_TASK -ci10 -cs10 $(echo $SRA)_trim.fastq temporary1_$SLURM_ARRAY_TASK_ID tmp_$SLURM_ARRAY_TASK_ID
                #Sort kmer database
